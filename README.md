# 👋 Hi, I'm Hitesh

🎓 Data Science Graduate Student (Germany)  
📍 Hamburg, Germany  
🚀 Passionate about Machine Learning, Time-Series Analysis & Data Analytics  

---

## 🧠 About Me

- 🎯 Seeking Working Student roles in Data Science / ML / Analytics  
- 📊 Strong in Python, SQL, and predictive modeling  
- ⚙️ Experienced in building end-to-end ML pipelines  
- 📈 Turning raw data into actionable insights  

---

## 🛠️ Tech Stack

### 👨‍💻 Languages
![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![SQL](https://img.shields.io/badge/SQL-003B57?style=for-the-badge&logo=database&logoColor=white)
![R](https://img.shields.io/badge/R-276DC3?style=for-the-badge&logo=r&logoColor=white)

### 📊 Data Science & ML
![Pandas](https://img.shields.io/badge/Pandas-150458?style=for-the-badge&logo=pandas)
![NumPy](https://img.shields.io/badge/NumPy-013243?style=for-the-badge&logo=numpy)
![Scikit-learn](https://img.shields.io/badge/Scikit--learn-F7931E?style=for-the-badge&logo=scikitlearn&logoColor=white)
![Machine Learning](https://img.shields.io/badge/Machine_Learning-FF6F00?style=for-the-badge)
![Time Series](https://img.shields.io/badge/Time--Series-4CAF50?style=for-the-badge)

### 🗄️ Databases
![MySQL](https://img.shields.io/badge/MySQL-4479A1?style=for-the-badge&logo=mysql&logoColor=white)
![MongoDB](https://img.shields.io/badge/MongoDB-4EA94B?style=for-the-badge&logo=mongodb)

### 📈 Visualization & Tools
![Matplotlib](https://img.shields.io/badge/Matplotlib-11557C?style=for-the-badge)
![Seaborn](https://img.shields.io/badge/Seaborn-4C72B0?style=for-the-badge)
![Streamlit](https://img.shields.io/badge/Streamlit-FF4B4B?style=for-the-badge&logo=streamlit&logoColor=white)

---

## 🚀 Featured Projects

### 📦 Supply Chain Delivery Delay Prediction
- Built ML models on **180K+ records**  
- Achieved **MAE: 0.55 days**  
- Developed interactive dashboard for insights  

---

### 📈 Crypto Price Direction Prediction (Time-Series ML)
- Trained models on **500K+ records**  
- Improved accuracy from **52.12% → 52.65%**  
- Applied leakage-free time-series validation  

---

### 🚀 SpaceX Falcon 9 Landing Prediction (IBM Capstone)
- Built classification models to predict landing success  
- Performed EDA, feature engineering, and model comparison  
- Developed interactive dashboard using Plotly Dash  

---

### ✈️ Aircraft Performance Analytics Pipeline
- Built ETL pipeline integrating aircraft datasets into MySQL  
- Developed ML model for range prediction  
- Created Streamlit app for interactive analysis  

---

## 📊 GitHub Stats

![Hitesh's GitHub stats](https://github-readme-stats.vercel.app/api?username=hitesh175&show_icons=true&theme=default)

![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=hitesh175&layout=compact)

---

## 🔥 Contribution Streak

![GitHub Streak](https://streak-stats.demolab.com/?user=hitesh175)

---

## 📫 Connect With Me

- 📧 Email: Kanagalahitesh@gmail.com  
- 💼 LinkedIn: https://linkedin.com/in/hitesh-k-r  
- 🌐 Portfolio: https://hitesh17.netlify.app/  

---

⭐ If you like my work, feel free to explore my repositories!
